...
/*
 *   $Id: cpp_demo.cpp,v 1.21 2013/07/09 16:41:22 r-lyeh Exp $
 *
 *   $Log: cpp_demo.cpp,v $
 *   Revision 1.21  2013/07/09 16:41:22  r-lyeh
 *   renamed "str"fmt to fmt("str") because of C++11 operator""
 *
...
void testMAPMcppRoutines(double x)
{
	...
	MAPM m=x;
	printf("-------------------------\nFor x=%f:\n",x);
#define fmt(text) text "%.13f = %s\n"
	compare(fmt("x+0       "),x+0,m+0);
	compare(fmt("x+1       "),x+1,m+1);
	compare(fmt("3+x       "),3+x,3+m);
	compare(fmt("2+(-x)    "),2+(-x),2+(-m));
	compare(fmt("x-3.2     "),x-3.2,m-3.2);
	compare(fmt("x-3.2 (as string) "),x-3.2,m-"3.2");
	compare(fmt("3.2+(-x)  "),3.2+(-x),3.2+(-m));
	compare(fmt("1.5*x     "),1.5*x,1.5*m);
	compare(fmt("2/x       "),2/x,2/m);
	int y=(int)(6+10*x+0.5);
	MAPM n=6+10*m;
	compare(fmt("y=6+10*x  "),y,n);
	compare(fmt("y div 2   "),y/2,n.div(2));
	compare(fmt("y mod 2   "),y%2,n.rem(2));
	compare(fmt("y div -3  "),y/-3,n.div(-3));
	compare(fmt("y mod -3  "),y%-3,n.rem(-3));
	compare(fmt("-y div 3  "),(-y)/-3,(-n).div(-3));
	compare(fmt("-y mod 3  "),(-y)%-3,(-n).rem(-3));

	compare(fmt("fabs(x)   "),fabs(x),fabs(m));
	compare(fmt("sin(x)    "),sin(x),sin(m));
	compare(fmt("x.sin()   "),sin(x),m.sin());
	compare(fmt("cos(x)    "),cos(x),cos(m));
	compare(fmt("tan(x)    "),tan(x),tan(m));
	compare(fmt("cbrt(x)   "),cbrt_local(x),cbrt(m));
	compare(fmt("sinh(x)   "),sinh(x),sinh(m));
	compare(fmt("cosh(x)   "),cosh(x),cosh(m));
	compare(fmt("tanh(x)   "),tanh(x),tanh(m));
	compare(fmt("asinh(x)  "),asinh_local(x),asinh(m));

	if (fabs(m) < 1)
	  {
           compare(fmt("asin(x)   "),asin(x),asin(m));
	   compare(fmt("acos(x)   "),acos(x),acos(m));
	   compare(fmt("atan(x)   "),atan(x),atan(m));
	   compare(fmt("atan2(x,-0.5)"),atan2(x,-0.5),atan2(m,-0.5));
           compare(fmt("atanh(x)  "),atanh_local(x),atanh(m));
	  }

	if (m >= 1)
           compare(fmt("acosh(x)  "),acosh_local(x),acosh(m));

	if (m > 0)
	  {
	   compare(fmt("sqrt(x)   "),sqrt(x),sqrt(m));
	   compare(fmt("log(x)    "),log(x),log(m));
	   compare(fmt("pow(x,-2.93)"),pow(x,-2.93),pow(m,-2.93));
	   compare(fmt("x.pow(-2.93)"),pow(x,-2.93),m.pow(-2.93));
	  }

	compare(fmt("x.pow(2)  "),pow(x,2),m.ipow(2));
	compare(fmt("x.pow(-3) "),pow(x,-3),m.ipow(-3));
	compare(fmt("exp(x)    "),exp(x),exp(m));
